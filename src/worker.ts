/**
 * Cloudflare Workers MCP Server with Hono
 *
 * This file implements a Model Context Protocol (MCP) server using:
 * - Hono framework for HTTP routing
 * - @hono/mcp for Streamable HTTP Transport
 * - Twitter API v2 for social media integration
 *
 * Related classes:
 * - TwitterService (src/x/services.ts): Twitter API operations
 */
...
// MCP サーバーのインスタンス作成関数
function createMcpServer(twitterService: TwitterService): McpServer {
...
app.get('/', (c) => {
  return c.json({
    ...
    version: '1.0.0',
    status: 'running',
    platform: 'Cloudflare Workers with Hono + MCP',
...
app.all('/mcp', async (c) => {
  try {
    ...
    twitterService.initializeWithEnv(c.env);

    // Create MCP server with Twitter tools
    const mcpServer = createMcpServer(twitterService);

    // Initialize transport if not already created
    if (!mcpTransport) {
